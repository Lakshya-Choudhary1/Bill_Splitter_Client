...
const groupStore = create((set, get) => ({
     ...
     getGroupMembers: async(groupId) =>{
          try{
               ...
               toast.error(error.response?.data?.message || "Failed to fetch group members. Please try again.");
          }
     },
     getCurrentMember: async(groupId) =>{
          try{
               ...
               toast.error(error.response?.data?.message || "Failed to fetch current member. Please try again.");
          }
     },
     addMember : async(groupId,memberId) =>{
          try{
               ///:groupId/member/:memberId
               const response = await axiosInstance.post(`/invitation/group/${groupId}/invite`, { memberId });
     ...
     removeMember : async(groupId,memberId)=>{
          try{
               ...
               if(response.data.success){
                    set((state) => ({
                         selectedGroupMembers: state.selectedGroupMembers.filter((member) => member.user_id !== memberId),
                    }));
